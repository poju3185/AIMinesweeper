...
class Sentence:
    ...
    def mark_mine(self, cell):
        ...
        if cell in self.cells:
            self.mines.add(cell)
            if len(self.mines) == self.count:
                self.safes = self.cells - self.mines
    ...
    def mark_safe(self, cell):
        ...
        if cell in self.cells:
            ...
            if len(self.safes) == 8 - self.count:
                self.mines = self.cells - self.safes
...
class MinesweeperAI:
    ...
    def add_knowledge(self, cell, count):
        ...
        for r in range(self.height):
            for c in range(self.width):
                if (
                    (r, c) in self.moves_made
                    or (r, c) in self.mines
                    or (r, c) in self.safes
                ):
                    continue
                neighbors = self.get_neighbors((r, c))
                new_count = 0
                for nr, nc in neighbors:
                    if (nr, nc) not in self.mines and (nr, nc) not in self.safes:
                        break
                    if (nr, nc) in self.mines:
                        new_count += 1
                else:
                    self.add_knowledge((r, c), new_count)
    ...
    def create_sentence(self, cell, count) -> Sentence:
        new_cells = set()
        neighbors = self.get_neighbors(cell)
        for nr, nc in neighbors:
            new_cells.add((nr, nc))
        return Sentence(new_cells, count)
    ...
    def make_random_move(self):
        ...
        while (r, c) in self.moves_made or (r, c) in self.mines:
            r = random.randrange(self.height)
            c = random.randrange(self.width)
        return (r, c)
